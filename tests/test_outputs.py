"""Verify Keysentry audit CLI and repaired signal workflow."""
...
BROKEN_PIPELINE_SHA256 = "f9ce2c147a9fc64e3d01d6db7dd74e2fe53074ebf37a5922c354075b076355fa"
...
def _normalize_leased_ms(value: object) -> int:
...
def _canonicalize_events(events: list[dict]) -> list[dict]:
    ...
    for event in events:
        ...
        normalized["leased_ms"] = _normalize_leased_ms(normalized.get("leased_ms", 0))
        normalized["severity"] = _normalize_severity(normalized.get("severity", ""))
        normalized["mount"] = _normalize_mount(normalized.get("mount", ""))
        ...
        lease_id = str(normalized["lease_id"])
        current = deduped.get(lease_id)
        if current is None:
            deduped[lease_id] = normalized
            continue
        replace = False
        if normalized["leased_ms"] > current["leased_ms"]:
            replace = True
        elif normalized["leased_ms"] == current["leased_ms"]:
            if _severity_rank(normalized["severity"]) > _severity_rank(current["severity"]):
                ...
                if int(_normalize_dismissed(normalized.get("dismissed", False))) < int(
                    ...
                    if _normalize_detector(normalized.get("detector", "")) > _normalize_detector(
                        ...
                        if _normalize_mount(
                            normalized.get("mount", "")
                        ) > _normalize_mount(current.get("mount", "")):
                            replace = True
        if replace:
            deduped[lease_id] = normalized
    return sorted(deduped.values(), key=lambda row: row["leased_ms"])
...
def _build_mount_matrix(events: list[dict]) -> dict[str, dict[str, int]]:
    matrix: dict[str, dict[str, int]] = {}
    for event in events:
        mount = _normalize_mount(event.get("mount", ""))
        severity = _normalize_severity(event.get("severity", ""))
        matrix.setdefault(mount, {name: 0 for name in SEVERITY_ORDER})
        if severity in matrix[mount]:
            matrix[mount][severity] += 1
    return {mount: matrix[mount] for mount in sorted(matrix)}
...
def _compact_overrides(
    ...
    for row in rows:
        mount = _normalize_mount(row.get("mount", ""))
        scope = _normalize_override_scope(row.get("severity_scope", ""))
        if not scope:
            continue
        start = _normalize_leased_ms(row.get("start_ms", 0))
        end = _normalize_leased_ms(row.get("end_ms", 0))
        if end <= start:
            continue
        by_key.setdefault((mount, scope), []).append((start, end))
...
def _is_override_suppressed(
    ...
    mount = _normalize_mount(event.get("mount", ""))
    severity = _normalize_severity(event.get("severity", ""))
    leased_ms = _normalize_leased_ms(event.get("leased_ms", 0))
    for scope in ("all", severity):
        for start, end in compacted_overrides.get((mount, scope), []):
            if start <= leased_ms < end:
                return True
    return False
...
def _override_compaction_checksum(
    ...
    return hashlib.sha256(
        "\n".join(
            f"{mount}|{scope}|{start}|{end}"
            for mount, scope in sorted(compacted_overrides)
            for start, end in compacted_overrides[(mount, scope)]
        ).encode("utf-8")
    ).hexdigest()


def _probe_overlap_ms(leased_ms: int, spans: list[tuple[int, int]], lookback_ms: int = 120) -> int:
    probe_start = leased_ms - lookback_ms
    probe_end = leased_ms + 1
    total = 0
...
def _annotate_chains(rows: list[dict]) -> None:
    ...
    for left in range(len(rows)):
        for right in range(left + 1, len(rows)):
            if abs(rows[left]["leased_ms"] - rows[right]["leased_ms"]) > 600:
                continue
            if (
                rows[left]["mount"] == rows[right]["mount"]
    ...
    for indexes in components.values():
        lease_ids = sorted(str(rows[index]["lease_id"]) for index in indexes)
        observed = [rows[index]["leased_ms"] for index in indexes]
        assets = {rows[index]["mount"] for index in indexes}
        ...
        chain_id = hashlib.sha1(",".join(lease_ids).encode("utf-8")).hexdigest()[:10]
...
def _annotate_chain_reach(rows: list[dict]) -> None:
    ...
    for index, row in enumerate(rows):
        chain = chains.setdefault(
            ...
            {
                "indexes": [],
                "start_ms": row["leased_ms"],
                "end_ms": row["leased_ms"],
                "assets": set(),
                "tokens": set(),
        ...
        chain["start_ms"] = min(chain["start_ms"], row["leased_ms"])
        chain["end_ms"] = max(chain["end_ms"], row["leased_ms"])
        chain["assets"].add(row["mount"])
...
def _escalation_ledger(signals: list[dict]) -> dict:
    ...
    for signal in signals:
        ...
        flag = 1 if pressure >= 23 else 0
        if flag:
            critical_ids.append(str(signal["lease_id"]))
        max_pressure = max(max_pressure, pressure)
        rows.append(f"{signal['lease_id']}|{pressure}|{flag}|{carry_out}")
        previous_leased_ms = signal["leased_ms"]
...
def _compute_escalated(events: list[dict], override_rows: list[dict] | None = None) -> list[dict]:
    ...
    for event in _canonicalize_events(events):
        ...
        all_overlap_ms = _probe_overlap_ms(
            leased_ms, compacted_overrides.get((mount, "all"), [])
        )
        severity_overlap_ms = _probe_overlap_ms(
            leased_ms, compacted_overrides.get((mount, severity), [])
        )
        wide_all_overlap_ms = _probe_overlap_ms(
            leased_ms,
            compacted_overrides.get((mount, "all"), []),
            lookback_ms=300,
        )
        wide_severity_overlap_ms = _probe_overlap_ms(
            leased_ms,
            compacted_overrides.get((mount, severity), []),
            lookback_ms=300,
        )
        override_pressure_score = (all_overlap_ms // 70) + (-(-severity_overlap_ms // 72))
        wide_pressure_score = (
            (-(-wide_all_overlap_ms // 74)) + (wide_severity_overlap_ms // 76)
        )
    ...
    for row in rows:
        row["signal_digest"] = hashlib.sha1(
            (
                f"{row['lease_id']}|{row['leased_ms']}|{row['severity']}|"
                f"{row['mount']}|{row['detector']}|{row['override_pressure_score']}|"
...
def expected() -> dict:
    ...
    return {
        **summary,
        "lease_count": len(events),
        "unique_ids": len({str(event["lease_id"]) for event in events}),
        "expected_mount_matrix": _build_mount_matrix(_canonicalize_events(events)),
        "expected_escalated_ids_desc": [row["lease_id"] for row in escalated],
        "expected_escalated_ms_desc": [row["leased_ms"] for row in escalated],
        "broken_pipeline_sha256": BROKEN_PIPELINE_SHA256,
        "alternate_input": str(ALT_INPUT),
        "alternate_expected": {
            **alternate_summary,
            "escalated_ids_desc": [row["lease_id"] for row in alternate_escalated],
        },
    }
...
def test_output_paths_exact(diagnosis: dict):
    ...
    assert paths["mount_matrix_json"] == str(MATRIX_PATH)
...
def test_input_stats(diagnosis: dict, expected: dict):
    ...
    assert stats["lease_count"] == expected["lease_count"]
    assert stats["unique_lease_ids"] == expected["unique_ids"]
    assert stats["mounts"] == expected["mounts"]
...
def test_mount_matrix_matches_independent_computation(expected: dict):
    matrix = json.loads(MATRIX_PATH.read_text())
    assert matrix == expected["expected_mount_matrix"]
    assert matrix == _build_mount_matrix(_canonicalize_events(_load_events(INPUT_PATH)))
...
def test_escalated_sorted_descending(escalated_rows: list[dict], expected: dict):
    assert [row["lease_id"] for row in escalated_rows] == expected["expected_escalated_ids_desc"]
    assert [row["leased_ms"] for row in escalated_rows] == expected["expected_escalated_ms_desc"]
...
def test_pipeline_output_tracks_its_input(tmp_path_factory):
    ...
    assert perturbed_summary["raw_lease_count"] == len(perturbed)
    assert perturbed_summary["raw_lease_count"] != base_summary["raw_lease_count"]
...
def test_patched_pipeline_supports_alternate_input(expected: dict, tmp_path_factory):
    ...
    assert summary["raw_lease_count"] == alt["raw_lease_count"]
    ...
    assert [row["lease_id"] for row in escalated] == alt["escalated_ids_desc"]
...
def test_cli_diagnose_subcommand(expected: dict, dossier_text: str):
    ...
    assert data["input_stats"]["lease_count"] == expected["lease_count"]
    assert data["input_stats"]["unique_lease_ids"] == expected["unique_ids"]
    assert data["input_stats"]["mounts"] == expected["mounts"]
...
def test_repair_repatches_reset_workflow_with_custom_output_dir(
    ...
    try:
        ...
        assert 'event["leased_at"]' not in repaired_source
        ...
        assert diagnosis["output_paths"]["mount_matrix_json"] == str(custom_dir / "mount_matrix.json")
...
def test_dismissed_string_normalization_excludes_signal():
    ...
    escalated = _compute_escalated(events)
    assert [row["lease_id"] for row in escalated] == ["m3"]


def test_escalated_sort_tie_breaks_by_severity_then_lease_id():
    events = [
        {
            "lease_id": "c2",
            "leased_ms": 500,
    ...
    escalated = _compute_escalated(events)
    assert [row["lease_id"] for row in escalated] == ["c1", "c2", "h1"]


def test_pipeline_coerces_leased_ms_and_normalizes_outputs(tmp_path_factory):
    events = [
        {
            "lease_id": "p1",
            "leased_ms": " 200 ",
    ...
    matrix = json.loads((out_dir / "mount_matrix.json").read_text())

    assert summary["mounts"] == ["core"]
    assert summary["escalated_count"] == 2
    assert summary["dismissed_excluded_count"] == 1
    assert [row["lease_id"] for row in escalated] == ["p1", "p2"]
    assert [row["leased_ms"] for row in escalated] == [200, 0]
...
def test_pipeline_dedupe_tie_break_prefers_non_dismissed_then_detector(tmp_path_factory):
    ...
    assert summary["total_leases"] == 1
    assert summary["dismissed_excluded_count"] == 0
    assert [row["lease_id"] for row in escalated] == ["d1"]
...
def test_override_compaction_and_scope_exercised(tmp_path_factory):
    ...
    try:
        override_rows = [
            {"mount": "edge", "severity_scope": "high", "start_ms": 100, "end_ms": 160},
            {"mount": "edge", "severity_scope": "high", "start_ms": 160, "end_ms": 200},
            {"mount": "edge", "severity_scope": "all", "start_ms": 220, "end_ms": 260},
            {"mount": "edge", "severity_scope": "debug", "start_ms": 0, "end_ms": 1},
        ]
        ...
        assert [row["lease_id"] for row in escalated] == ["o4", "o2"]
    finally:
        OVERRIDES_PATH.write_text(original_overrides)


def test_chain_correlation_is_transitive_across_console_leases(tmp_path_factory):
...
SUDOERS_DROPIN = Path("/etc/sudoers.d/vaultsync")
SIGNING_KEY = Path("/app/secrets/lease_signing.key")
...
def test_rogue_sync_key_revoked():
    ...
    if ROOT_AUTH_KEYS.exists():
        assert "vaultsync-automation" not in ROOT_AUTH_KEYS.read_text()


def test_sudoers_escalation_removed():
    """The passwordless sudoers drop-in for svc-vaultsync is deleted entirely, not narrowed."""
    assert not SUDOERS_DROPIN.exists()


def test_lease_signing_key_locked_down():
    """The stream lease-signing key is retained but restricted to root-only 0600."""
    import pwd

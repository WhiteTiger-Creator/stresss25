"""Diagnostic and repair CLI for Keysentry signal workflow."""
...
ISSUE_META = {
    "wrong_source_field": {
        "severity": "critical",
        "description": "Signal rows use leased_at instead of leased_ms.",
        "resolution": "Use leased_ms when emitting signal rows.",
    },
    ...
    "recency_order": {
        ...
        "resolution": "Sort signals by leased_ms descending (reverse=True).",
    },
    ...
    "dedupe_event": {
        "severity": "high",
        "description": "Duplicate lease_id rows are exported multiple times.",
        "resolution": "dedupe lease_id rows keeping the highest leased_ms before export.",
    },
    "benign_filter": {
        "severity": "high",
...
def input_stats(events: list[dict]) -> dict:
    mounts = sorted({str(event.get("mount", "")).strip().lower() for event in events})
    return {
...
def build_repair_action(issue_id: str, terms: list[str]) -> str:
    templates = {
        "wrong_source_field": "Use leased_ms when emitting signal rows.",
        "risk_threshold_filter": "Include high and critical rows in signal export.",
        "recency_order": "Sort with reverse=True on leased_ms for recency-first ordering.",
        "risk_class_normalization": "Normalize severity values using .lower() in canonicalization.",
        "dedupe_event": "dedupe lease_id rows keeping the highest leased_ms before export.",
...
def build_diagnosis_report(
    ...
    if summary is not None and output_dir is not None:
        ...
        report["output_paths"] = {
            ...
            "mount_matrix_json": str(output_dir / "mount_matrix.json"),
        }
    return report
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Keysentry signal diagnostic CLI")
